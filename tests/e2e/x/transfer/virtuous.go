...
var _ = e2e.DescribeXChain("[Virtuous Transfer Tx DJTX]", func() {
	ginkgo.It("can issue a virtuous transfer tx for DJTX asset",
		// use this for filtering tests by labels
		// ref. https://onsi.github.io/ginkgo/#spec-labels
		ginkgo.Label(
			"require-network-runner",
			"x",
			"virtuous-transfer-tx-djtx",
		),
		func() {
			...
			runFunc := func(round int) {
				...
				for i, w := range wallets {
					...
					bal := balances[djtxAssetID]
					testBalances = append(testBalances, bal)

					fmt.Printf(`CURRENT BALANCE %21d DJTX (SHORT ADDRESS %q)
`,
						bal,
				...
				ginkgo.By("Value-Chain transfer with wrong amount must fail", func() {
...
FROM [%q]
SENDER    CURRENT BALANCE     : %21d DJTX
SENDER    NEW BALANCE (AFTER) : %21d DJTX

TRANSFER AMOUNT FROM SENDER   : %21d DJTX

TO [%q]
RECEIVER  CURRENT BALANCE     : %21d DJTX
RECEIVER  NEW BALANCE (AFTER) : %21d DJTX
===
`,
					shortAddrs[fromIdx],
					senderOrigBal,
