...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/network"
	"github.com/lasthyphen/dijetsnodesgo/snow"
	"github.com/lasthyphen/dijetsnodesgo/snow/consensus/snowball"
	"github.com/lasthyphen/dijetsnodesgo/snow/engine/dijets/state"
	...
	"github.com/lasthyphen/dijetsnodesgo/snow/engine/common/tracker"
	"github.com/lasthyphen/dijetsnodesgo/snow/engine/snowman/block"
	"github.com/lasthyphen/dijetsnodesgo/snow/engine/snowman/syncer"
	"github.com/lasthyphen/dijetsnodesgo/snow/networking/handler"
	"github.com/lasthyphen/dijetsnodesgo/snow/networking/router"
	"github.com/lasthyphen/dijetsnodesgo/snow/networking/sender"
	...
	"github.com/lasthyphen/dijetsnodesgo/utils/buffer"
	...
	"github.com/lasthyphen/dijetsnodesgo/vms/metervm"
	"github.com/lasthyphen/dijetsnodesgo/vms/proposervm"
	"github.com/lasthyphen/dijetsnodesgo/vms/tracedvm"

	dbManager "github.com/lasthyphen/dijetsnodesgo/database/manager"
	timetracker "github.com/lasthyphen/dijetsnodesgo/snow/networking/tracker"

	avcon "github.com/lasthyphen/dijetsnodesgo/snow/consensus/dijets"
	aveng "github.com/lasthyphen/dijetsnodesgo/snow/engine/dijets"
	avbootstrap "github.com/lasthyphen/dijetsnodesgo/snow/engine/dijets/bootstrap"
	avagetter "github.com/lasthyphen/dijetsnodesgo/snow/engine/dijets/getter"

	smcon "github.com/lasthyphen/dijetsnodesgo/snow/consensus/snowman"
	smeng "github.com/lasthyphen/dijetsnodesgo/snow/engine/snowman"
	smbootstrap "github.com/lasthyphen/dijetsnodesgo/snow/engine/snowman/bootstrap"
	snowgetter "github.com/lasthyphen/dijetsnodesgo/snow/engine/snowman/getter"
)
...
var (
	...
	errUnknownVMType    = errors.New("the vm should have type dijets.DAGVM or snowman.ChainVM")
...
func (m *manager) QueueChainCreation(chainParams ChainParameters) {
	...
	if !addedChain {
		m.Log.Debug("skipping dijets chain creation",
			zap.String("reason", "dijets chain already staged"),
	...
	if ok := m.chainsQueue.PushRight(chainParams); !ok {
		m.Log.Warn("skipping dijets chain creation",
...
func (m *manager) createChain(chainParams ChainParameters) {
	m.Log.Info("creating dijets chain",
		zap.Stringer("subnetID", chainParams.SubnetID),
		zap.Stringer("chainID", chainParams.ID),
		zap.Stringer("vmID", chainParams.VMID),
	)
	...
	if err != nil {
		if m.CriticalChains.Contains(chainParams.ID) {
			...
			m.Log.Fatal("error creating required dijets chain",
				zap.Stringer("dijets subnetID", chainParams.SubnetID),
				zap.Stringer("dijets chainID", chainParams.ID),
				zap.Stringer("vmID", chainParams.VMID),
				zap.Error(err),
			)
		...
		if err != nil {
			m.Log.Error("failed to register failing health check",
				zap.Stringer("dijets subnetID", chainParams.SubnetID),
				zap.Stringer("dijets chainID", chainParams.ID),
				zap.String("dijets chainAlias", chainAlias),
				zap.Stringer("dijets vmID", chainParams.VMID),
				zap.Error(err),
			)
		}
		return
...
func (m *manager) StartChainCreator(platform ChainParameters) {
	...
	m.Log.Info("starting chain creator and looking for whitelisted ECCs")
