...
func TestGetChainConfigsFromFiles(t *testing.T) {
	tests := map[string]struct {
		...
		"valid alias": {
			configs:  map[string]string{"U": "hello", "V": "world"},
			upgrades: map[string]string{"U": "upgradess"},
			expected: func() map[string]chains.ChainConfig {
				m := map[string]chains.ChainConfig{}
				m["U"] = chains.ChainConfig{Config: []byte("hello"), Upgrade: []byte("upgradess")}
				m["V"] = chains.ChainConfig{Config: []byte("world"), Upgrade: []byte(nil)}
...
func TestGetChainConfigsDirNotExist(t *testing.T) {
	tests := map[string]struct {
		...
		"full structure": {
			...
			expected:  map[string]chains.ChainConfig{"U": {Config: []byte("hello"), Upgrade: []byte(nil)}},
		},
	}
...
func TestSetChainConfigDefaultDir(t *testing.T) {
	...
	chainsDir := filepath.Join(defaultChainConfigDir, "U")
	setupFile(t, chainsDir, chainConfigFileName+".ex", "helloworld")
	chainConfigs, err := getChainConfigs(v)
	require.NoError(err)
	expected := map[string]chains.ChainConfig{"U": {Config: []byte("helloworld"), Upgrade: []byte(nil)}}
...
func TestGetChainConfigsFromFlags(t *testing.T) {
	tests := map[string]struct {
		...
		"valid alias": {
			fullConfigs: map[string]chains.ChainConfig{
				"U": {Config: []byte("hello"), Upgrade: []byte("upgradess")},
				"V": {Config: []byte("world"), Upgrade: []byte(nil)},
			},
			expected: func() map[string]chains.ChainConfig {
				m := map[string]chains.ChainConfig{}
				m["U"] = chains.ChainConfig{Config: []byte("hello"), Upgrade: []byte("upgradess")}
