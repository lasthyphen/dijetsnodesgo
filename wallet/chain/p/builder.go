...
func (b *builder) NewBaseTx(
	...
	djtx.SortTransferableOutputs(outputs, txs.Codec) // sort the outputs
...
func (b *builder) NewAddValidatorTx(
	...
	toBurn := map[ids.ID]uint64{
		djtxAssetID: b.backend.AddPrimaryNetworkValidatorFee(),
	}
	toStake := map[ids.ID]uint64{
		djtxAssetID: vdr.Wght,
	}
	ops := common.NewOptions(options)
...
func (b *builder) NewAddSubnetValidatorTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.DJTXAssetID(): b.backend.AddSubnetValidatorFee(),
	}
	toStake := map[ids.ID]uint64{}
	ops := common.NewOptions(options)
...
func (b *builder) NewAddDelegatorTx(
	...
	djtxAssetID := b.backend.DJTXAssetID()
	toBurn := map[ids.ID]uint64{
		djtxAssetID: b.backend.AddPrimaryNetworkDelegatorFee(),
	}
	toStake := map[ids.ID]uint64{
		b.backend.DJTXAssetID(): vdr.Wght,
	}
	ops := common.NewOptions(options)
	inputs, baseOutputs, stakeOutputs, err := b.spend(toBurn, toStake, ops)
	if err != nil {
...
func (b *builder) NewCreateChainTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.DJTXAssetID(): b.backend.CreateBlockchainTxFee(),
	}
	toStake := map[ids.ID]uint64{}
	ops := common.NewOptions(options)
	inputs, outputs, _, err := b.spend(toBurn, toStake, ops)
...
func (b *builder) NewImportTx(
	...
	djtx.SortTransferableOutputs(outputs, txs.Codec) // sort imported outputs
	return &txs.ImportTx{
		BaseTx: txs.BaseTx{BaseTx: djtx.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: constants.PlatformChainID,
...
func (b *builder) NewExportTx(
	...
	djtx.SortTransferableOutputs(outputs, txs.Codec) // sort exported outputs
	return &txs.ExportTx{
		BaseTx: txs.BaseTx{BaseTx: djtx.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: constants.PlatformChainID,
...
func (b *builder) NewTransformSubnetTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.DJTXAssetID(): b.backend.TransformSubnetTxFee(),
...
func (b *builder) NewAddPermissionlessValidatorTx(
	...
	if vdr.Subnet == constants.PrimaryNetworkID {
		toBurn[djtxAssetID] = b.backend.AddPrimaryNetworkValidatorFee()
	} else {
		toBurn[djtxAssetID] = b.backend.AddSubnetValidatorFee()
	}
	toStake := map[ids.ID]uint64{
		assetID: vdr.Wght,
	}
...
func (b *builder) NewAddPermissionlessDelegatorTx(
	...
	toBurn := map[ids.ID]uint64{}
	if vdr.Subnet == constants.PrimaryNetworkID {
		toBurn[djtxAssetID] = b.backend.AddPrimaryNetworkDelegatorFee()
	} else {
		toBurn[djtxAssetID] = b.backend.AddSubnetDelegatorFee()
	}
	toStake := map[ids.ID]uint64{
		assetID: vdr.Wght,
	}
...
func (b *builder) spend(
	...
	djtx.SortTransferableOutputs(changeOutputs, txs.Codec) // sort the change outputs
	djtx.SortTransferableOutputs(stakeOutputs, txs.Codec)  // sort stake outputs
