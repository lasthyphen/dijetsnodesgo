...
import (
	...
	safemath "github.com/lasthyphen/dijetsnodesgo/utils/math"
)
...
func (s *Service) GetAddressTxs(_ *http.Request, args *GetAddressTxsArgs, reply *GetAddressTxsReply) error {
	...
	// Parse to address
	address, err := djtx.ParseServiceAddress(s.vm, args.Address)
	if err != nil {
...
func (s *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, reply *api.GetUTXOsReply) error {
	...
	addrSet, err := djtx.ParseServiceAddresses(s.vm, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = djtx.ParseServiceAddress(s.vm, args.StartIndex.Address)
		if err != nil {
...
func (s *Service) GetBalance(_ *http.Request, args *GetBalanceArgs, reply *GetBalanceReply) error {
	...
	addr, err := djtx.ParseServiceAddress(s.vm, args.Address)
	if err != nil {
	...
	utxos, err := djtx.GetAllUTXOs(s.vm.state, addrSet)
	if err != nil {
		return fmt.Errorf("problem retrieving UTXOs: %w", err)
	}
...
func (s *Service) CreateAsset(_ *http.Request, args *CreateAssetArgs, reply *AssetIDChangeAddr) error {
	...
	fromAddrs, err := djtx.ParseServiceAddresses(s.vm, args.From)
	if err != nil {
		return err
	}
	...
	if amountSpent := amountsSpent[s.vm.feeAssetID]; amountSpent > s.vm.CreateAssetTxFee {
		outs = append(outs, &djtx.TransferableOutput{
			Asset: djtx.Asset{ID: s.vm.feeAssetID},
	...
	for _, holder := range args.InitialHolders {
		addr, err := djtx.ParseServiceAddress(s.vm, holder.Address)
		if err != nil {
			return err
		}
	...
	for _, owner := range args.MinterSets {
		...
		minterAddrsSet, err := djtx.ParseServiceAddresses(s.vm, owner.Minters)
		if err != nil {
			return err
		}
...
func (s *Service) SendMultiple(_ *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	for _, output := range args.Outputs {
		...
		to, err := djtx.ParseServiceAddress(s.vm, output.To)
		if err != nil {
	...
	djtx.SortTransferableOutputs(outs, s.vm.parser.Codec())
...
func (s *Service) Mint(_ *http.Request, args *MintArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	to, err := djtx.ParseServiceAddress(s.vm, args.To)
	if err != nil {
