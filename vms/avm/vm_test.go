...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/database/mockdb"
...
func GetDJTXTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *txs.Tx {
	return GetCreateTxFromGenesisTest(tb, genesisBytes, "DJT")
}
...
func NewTx(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	return NewTxWithAsset(t, genesisBytes, vm, "DJT")
}
...
func TestTxVerifyAfterVerifyAncestorTx(t *testing.T) {
	...
	secondTx := issueTxs[2]
	key := keys[0]
	firstTxDescendant := &txs.Tx{Unsigned: &txs.BaseTx{BaseTx: djtx.BaseTx{
		NetworkID:    networkID,
		BlockchainID: chainID,
		Ins: []*djtx.TransferableInput{{
			UTXOID: djtx.UTXOID{
