...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/vms/avm/states"
	"github.com/lasthyphen/dijetsnodesgo/vms/avm/txs"
	"github.com/lasthyphen/dijetsnodesgo/vms/components/djtx"
	"github.com/lasthyphen/dijetsnodesgo/vms/components/index"
	...
	extensions "github.com/lasthyphen/dijetsnodesgo/vms/avm/fxs"
)
...
func (vm *VM) initGenesis(genesisBytes []byte) error {
	...
	for index, genesisTx := range genesis.Txs {
		...
		if index == 0 {
			vm.ctx.Log.Info("Dijets network fee asset is established",
...
func (vm *VM) initState(tx txs.Tx) error {
	txID := tx.ID()
	vm.ctx.Log.Info("initializing DJT coin as genesis asset",
...
func (vm *VM) verifyTransferOfUTXO(utx txs.UnsignedTx, in *djtx.TransferableInput, cred verify.Verifiable, utxo *djtx.UTXO) error {
