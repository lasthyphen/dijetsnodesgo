...
type environment struct {
	...
	fx             fx.Fx
	state          state.State
	atomicUTXOs    djtx.AtomicUTXOManager
...
func newEnvironment(t *testing.T) *environment {
	...
	res.atomicUTXOs = djtx.NewAtomicUTXOManager(res.ctx.SharedMemory, txs.Codec)
...
func defaultConfig() *config.Config {
	...
	return &config.Config{
		...
		MinValidatorStake:      5 * units.MilliDjtx,
		MaxValidatorStake:      500 * units.MilliDjtx,
		MinDelegatorStake:      1 * units.MilliDjtx,
