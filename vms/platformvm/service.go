...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/vms/components/keystore"
	...
	"github.com/lasthyphen/dijetsnodesgo/vms/platformvm/txs/builder"
	"github.com/lasthyphen/dijetsnodesgo/vms/platformvm/txs/executor"
	"github.com/lasthyphen/dijetsnodesgo/vms/secp256k1fx"

	platformapi "github.com/lasthyphen/dijetsnodesgo/vms/platformvm/api"
)
...
type Service struct {
	vm          *VM
	addrManager djtx.AddressManager
}
...
func (s *Service) ExportKey(_ *http.Request, args *ExportKeyArgs, reply *ExportKeyReply) error {
	...
	address, err := djtx.ParseServiceAddress(s.addrManager, args.Address)
	if err != nil {
...
// Note: We explicitly duplicate DJTX out of the maps to ensure backwards
// compatibility.
...
func (s *Service) GetBalance(_ *http.Request, args *GetBalanceRequest, response *GetBalanceResponse) error {
	...
	// Parse to address
	addrs, err := djtx.ParseServiceAddresses(s.addrManager, args.Addresses)
	if err != nil {
		return err
	}

	utxos, err := djtx.GetAllUTXOs(s.vm.state, addrs)
	if err != nil {
...
utxoFor:
	...
	response.Balance = response.Balances[s.vm.ctx.DJTXAssetID]
	response.Unlocked = response.Unlockeds[s.vm.ctx.DJTXAssetID]
	response.LockedStakeable = response.LockedStakeables[s.vm.ctx.DJTXAssetID]
	response.LockedNotStakeable = response.LockedNotStakeables[s.vm.ctx.DJTXAssetID]
	return nil
}
...
func (s *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, response *api.GetUTXOsReply) error {
	...
	addrSet, err := djtx.ParseServiceAddresses(s.addrManager, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = djtx.ParseServiceAddress(s.addrManager, args.StartIndex.Address)
		if err != nil {
...
func (s *Service) GetStakingAssetID(_ *http.Request, args *GetStakingAssetIDArgs, response *GetStakingAssetIDResponse) error {
	...
	if args.SubnetID == constants.PrimaryNetworkID {
		response.AssetID = s.vm.ctx.DJTXAssetID
		return nil
	}
...
func (s *Service) AddValidator(_ *http.Request, args *AddValidatorArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := djtx.ParseServiceAddresses(s.addrManager, args.From)
	if err != nil {
		return err
	}

	// Parse the reward address
	rewardAddress, err := djtx.ParseServiceAddress(s.addrManager, args.RewardAddress)
	if err != nil {
	...
	if args.ChangeAddr != "" {
		changeAddr, err = djtx.ParseServiceAddress(s.addrManager, args.ChangeAddr)
		if err != nil {
...
func (s *Service) CreateSubnet(_ *http.Request, args *CreateSubnetArgs, response *api.JSONTxIDChangeAddr) error {
	...
	controlKeys, err := djtx.ParseServiceAddresses(s.addrManager, args.ControlKeys)
	if err != nil {
		return err
	}
...
func (s *Service) ExportDJTX(_ *http.Request, args *ExportDJTXArgs, response *api.JSONTxIDChangeAddr) error {
	s.vm.ctx.Log.Debug("Platform: ExportDJTX called")
...
func (s *Service) ImportDJTX(_ *http.Request, args *ImportDJTXArgs, response *api.JSONTxIDChangeAddr) error {
	s.vm.ctx.Log.Debug("Platform: ImportDJTX called")
	...
	to, err := djtx.ParseServiceAddress(s.addrManager, args.To)
...
func (s *Service) GetStake(_ *http.Request, args *GetStakeArgs, response *GetStakeReply) error {
	...
	response.Staked = response.Stakeds[s.vm.ctx.DJTXAssetID]
...
func getStakeHelper(tx *txs.Tx, addrs set.Set[ids.ShortID], totalAmountStaked map[ids.ID]uint64) []djtx.TransferableOutput {
	...
	stake := staker.Stake()
	stakedOuts := make([]djtx.TransferableOutput, 0, len(stake))
