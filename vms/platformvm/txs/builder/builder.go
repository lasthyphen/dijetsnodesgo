...
func New(
	...
	fx fx.Fx,
	state state.Chain,
	atomicUTXOManager djtx.AtomicUTXOManager,
...
func (b *builder) NewImportTx(
	...
	importedDJTX := importedAmounts[b.ctx.DJTXAssetID]

	ins := []*djtx.TransferableInput{}
	outs := []*djtx.TransferableOutput{}
	switch {
	case importedDJTX < b.cfg.TxFee: // imported amount goes toward paying tx fee
		var baseSigners [][]*crypto.PrivateKeySECP256K1R
		ins, outs, _, baseSigners, err = b.Spend(keys, 0, b.cfg.TxFee-importedDJTX, changeAddr)
		if err != nil {
			return nil, fmt.Errorf("couldn't generate tx inputs/outputs: %w", err)
		}
		signers = append(baseSigners, signers...)
		delete(importedAmounts, b.ctx.DJTXAssetID)
	case importedDJTX == b.cfg.TxFee:
		delete(importedAmounts, b.ctx.DJTXAssetID)
	default:
		importedAmounts[b.ctx.DJTXAssetID] -= b.cfg.TxFee
	}
	...
	djtx.SortTransferableOutputs(outs, txs.Codec) // sort imported outputs
...
// TODO: should support other assets than DJTX
func (b *builder) NewExportTx(
	...
	utx := &txs.ExportTx{
		...
		ExportedOutputs: []*djtx.TransferableOutput{{ // Exported to X-Chain
			Asset: djtx.Asset{ID: b.ctx.DJTXAssetID},
			Out: &secp256k1fx.TransferOutput{
				Amt: amount,
				OutputOwners: secp256k1fx.OutputOwners{
					Locktime:  0,
