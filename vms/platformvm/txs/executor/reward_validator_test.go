...
func TestRewardValidatorTxExecuteOnCommit(t *testing.T) {
	...
	// Get old balances
	oldBalance, err := djtx.GetBalance(env.state, stakeOwners)
	require.NoError(err)
	...
	onCommitBalance, err := djtx.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardValidatorTxExecuteOnAbort(t *testing.T) {
	...
	onAbortBalance, err := djtx.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardDelegatorTxExecuteOnCommit(t *testing.T) {
	...
	oldVdrBalance, err := djtx.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	oldDelBalance, err := djtx.GetBalance(env.state, delDestSet)
	require.NoError(err)
	...
	commitVdrBalance, err := djtx.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	...
	commitDelBalance, err := djtx.GetBalance(env.state, delDestSet)
	require.NoError(err)
