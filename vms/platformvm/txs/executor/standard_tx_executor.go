...
func (e *StandardTxExecutor) CreateChainTx(tx *txs.CreateChainTx) error {
	...
	if err := e.FlowChecker.VerifySpend(
		...
		baseTxCreds,
		map[ids.ID]uint64{
			e.Ctx.DJTXAssetID: createBlockchainTxFee,
		},
	); err != nil {
		return err
	}
...
func (e *StandardTxExecutor) CreateSubnetTx(tx *txs.CreateSubnetTx) error {
	...
	if err := e.FlowChecker.VerifySpend(
		...
		e.Tx.Creds,
		map[ids.ID]uint64{
			e.Ctx.DJTXAssetID: createSubnetTxFee,
		},
	); err != nil {
		return err
	}
...
func (e *StandardTxExecutor) ImportTx(tx *txs.ImportTx) error {
	...
	if e.Bootstrapped.GetValue() {
		...
		if err := e.FlowChecker.VerifySpendUTXOs(
			...
			e.Tx.Creds,
			map[ids.ID]uint64{
				e.Ctx.DJTXAssetID: e.Config.TxFee,
			},
		); err != nil {
			return err
		}
...
func (e *StandardTxExecutor) TransformSubnetTx(tx *txs.TransformSubnetTx) error {
	...
	if err := e.Backend.FlowChecker.VerifySpend(
		...
		tx.Outs,
		baseTxCreds,
		// Invariant: [tx.AssetID != e.Ctx.DJTXAssetID]. This prevents the first
		//            entry in this map literal from being overwritten by the
		//            second entry.
		map[ids.ID]uint64{
			e.Ctx.DJTXAssetID: e.Config.TransformSubnetTxFee,
