...
func TestDiffRewardUTXO(t *testing.T) {
	...
	// Put a reward UTXO
	txID := ids.GenerateTestID()
	rewardUTXO := &djtx.UTXO{
		UTXOID: djtx.UTXOID{TxID: txID},
	}
...

	{
		...
		parentRewardUTXO := &djtx.UTXO{
			UTXOID: djtx.UTXOID{TxID: txID2},
		}
		state.EXPECT().GetRewardUTXOs(txID2).Return([]*djtx.UTXO{parentRewardUTXO}, nil).Times(1)
