...
func (d *diff) GetRewardUTXOs(txID ids.ID) ([]*djtx.UTXO, error) {
...
func (d *diff) AddRewardUTXO(txID ids.ID, utxo *djtx.UTXO) {
	if d.addedRewardUTXOs == nil {
		d.addedRewardUTXOs = make(map[ids.ID][]*djtx.UTXO)
	}
	d.addedRewardUTXOs[txID] = append(d.addedRewardUTXOs[txID], utxo)
}

func (d *diff) GetUTXO(utxoID ids.ID) (*djtx.UTXO, error) {
...
func (d *diff) AddUTXO(utxo *djtx.UTXO) {
