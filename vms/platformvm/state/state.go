...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/database/linkeddb"
...
func new(
	...
	utxoState, err := djtx.NewMeteredUTXOState(utxoDB, txs.GenesisCodec, metricsReg)
	if err != nil {
		return nil, err
	}
...
func (s *state) GetRewardUTXOs(txID ids.ID) ([]*djtx.UTXO, error) {
...
func (s *state) AddRewardUTXO(txID ids.ID, utxo *djtx.UTXO) {
	s.addedRewardUTXOs[txID] = append(s.addedRewardUTXOs[txID], utxo)
}

func (s *state) GetUTXO(utxoID ids.ID) (*djtx.UTXO, error) {
...
func (s *state) AddUTXO(utxo *djtx.UTXO) {
