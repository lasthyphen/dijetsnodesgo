...
func Consume(utxoDB state.UTXODeleter, ins []*djtx.TransferableInput) {
...
func (h *handler) Spend(
	...
	utxos, err := djtx.GetAllUTXOs(h.utxosReader, addrs) // The UTXOs controlled by [keys]
	if err != nil {
	...
	for _, utxo := range utxos {
		...
		if assetID := utxo.AssetID(); assetID != h.ctx.DJTXAssetID {
		...
		if !ok { // should never happen
			h.ctx.Log.Warn("wrong input type",
				zap.String("expectedType", "djtx.TransferableIn"),
	...
	for _, utxo := range utxos {
		// If we have consumed more DJTX than we are trying to stake,
		// and we have burned more DJTX than we need to,
		// then we have no need to consume more DJTX
	...
	djtx.SortTransferableOutputs(returnedOuts, txs.Codec) // sort outputs
	djtx.SortTransferableOutputs(stakedOuts, txs.Codec)   // sort outputs
