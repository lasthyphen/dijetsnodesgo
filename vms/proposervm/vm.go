...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/cache/metercacher"
	...
	"github.com/lasthyphen/dijetsnodesgo/vms/proposervm/proposer"
	"github.com/lasthyphen/dijetsnodesgo/vms/proposervm/scheduler"
	"github.com/lasthyphen/dijetsnodesgo/vms/proposervm/state"
	"github.com/lasthyphen/dijetsnodesgo/vms/proposervm/tree"

	statelessblock "github.com/lasthyphen/dijetsnodesgo/vms/proposervm/block"
)
...
func (vm *VM) repair(ctx context.Context, indexerState state.State) error {
	...
	if !vm.resetHeightIndexOngoing.GetValue() {
		...
		case nil:
			...
			if !shouldRepair {
				vm.ctx.Log.Info("Block height index for Dijets Utility Chain was successfully verified")
...
func (vm *VM) verifyAndRecordInnerBlk(ctx context.Context, blockCtx *block.Context, postFork PostForkBlock) error {
	...
	if shouldVerifyWithCtx {
		// This block needs to know the Method-Chain height during verification.
