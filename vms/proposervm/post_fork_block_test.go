...
func TestBlockVerify_PostForkBlock_PChainHeightChecks(t *testing.T) {
	...
	// child Method-Chain height must not precede parent Method-Chain height
	childSlb, err := block.Build(
		prntProBlk.ID(),
		childCoreBlk.Timestamp(),
	...
	if err := childProBlk.Verify(context.Background()); err == nil {
		t.Fatal("ProBlock's Method-Chain-Height cannot be lower than parent ProBlock's one")
	} else if err == nil {
