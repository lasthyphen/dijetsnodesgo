...
# Build Coreth
build_args="$race"
echo "Building Dijets Platform Services @ ${coreth_version} ..."
cd "$coreth_path"
go build $build_args -ldflags "-X github.com/lasthyphen/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
