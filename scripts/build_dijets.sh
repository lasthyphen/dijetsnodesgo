...
if version_lt "$(go_version)" "$go_version_minimum"; then
    echo "DijetsNodeGo requires Go >= $go_version_minimum, Go $(go_version) found." >&2
    exit 1
fi
...
go build $build_args -ldflags "-X github.com/lasthyphen/dijetsnodesgo/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
