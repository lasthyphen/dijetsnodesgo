...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/snow/consensus/dijets/poll"
...
func newTransitive(config Config) (*Transitive, error) {
	config.Ctx.Log.Info("initialising Dijets Consensus engine")
...
func (t *Transitive) GetVtx(ctx context.Context, vtxID ids.ID) (dijets.Vertex, error) {
...
// Returns true if [vtx] has been added to consensus (now or previously)
func (t *Transitive) issueFrom(ctx context.Context, nodeID ids.NodeID, vtx dijets.Vertex) (bool, error) {
	issued := true
...
// Assumes we have [vtx].
func (t *Transitive) issue(ctx context.Context, vtx dijets.Vertex) error {
	vtxID := vtx.ID()
