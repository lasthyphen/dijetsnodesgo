...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/utils/linkedhashmap"
	"github.com/lasthyphen/dijetsnodesgo/utils/logging"
	"github.com/lasthyphen/dijetsnodesgo/utils/metric"
...
func NewLatency(metricName, descriptionName string, log logging.Logger, namespace string, reg prometheus.Registerer) (Latency, error) {
	...
	l := &latency{
		...
		// e.g.,
		// "dijets_7y7zwo7XatqnX4dtTakLo32o7jkMX4XuDa26WaxbCXoCT1qKK_blks_processing" to count how blocks are currently processing
		...
		// e.g.,
		// "dijets_C_blks_accepted_count" to count how many "Observe" gets called -- count all "Accept"
		// "dijets_C_blks_accepted_sum" to count how many ns have elapsed since its issuance on acceptance
		// "dijets_C_blks_accepted_sum / dijets_C_blks_accepted_count" is the average block acceptance latency in ns
		// "dijets_C_blks_accepted_container_size_sum" to track cumulative sum of all accepted blocks' sizes
		// "dijets_C_blks_accepted_container_size_sum / dijets_C_blks_accepted_count" is the average block size
		...
		// e.g.,
		// "dijets_P_blks_rejected_count" to count how many "Observe" gets called -- count all "Reject"
		// "dijets_P_blks_rejected_sum" to count how many ns have elapsed since its issuance on rejection
		// "dijets_P_blks_accepted_sum / dijets_P_blks_accepted_count" is the average block acceptance latency in ns
		// "dijets_P_blks_accepted_container_size_sum" to track cumulative sum of all accepted blocks' sizes
		// "dijets_P_blks_accepted_container_size_sum / dijets_P_blks_accepted_count" is the average block size
