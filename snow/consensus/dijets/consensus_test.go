...
import (
	...
	"github.com/lasthyphen/dijetsnodesgo/utils/compare"
...
func AddTest(t *testing.T, factory Factory) {
	...
	if !avl.Finalized() {
		t.Fatal("An empty dijets instance is not finalized")
	}
...
func SplitVotingTest(t *testing.T, factory Factory) {
	...
	case err != nil:
		t.Fatal(err)
	case avl.Finalized(): // dijets shouldn't be finalized because the vertex transactions are still processing
		t.Fatalf("An dijets instance finalized too late")
...
func TransactionVertexTest(t *testing.T, factory Factory) {
	...
	case !avl.Finalized():
		t.Fatal("expected finalized dijets instance")
